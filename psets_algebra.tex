...
色々な数学的帰納法.
\pbenumex{
    $\bm{N}\backslash \{0\}$ を $\bm{Z}^+$ で表す.\footnote[1]{本来, 整数が導入された後に使う記号のため, 後で再定義する.} このとき, 次の問に答えよ.
}{
\item 次の条件を満たす $\bm{Z}^+$ の部分集合 $S$ を考える.
\begin{align}
    &1 \in S\\
    &\forall n \in \bm{Z}^+ [n \in S \implies n + 1 \in S]
\end{align}
$S = \bm{Z}^+$ を満たすことを示せ.
\item $\bm{Z}^+$ の元の各々に対し, 命題 $P(n)$ が与えられたとし, それについて次の二つのことが示されたとする.
\setcounter{equation}{0}
\begin{align}
    &P(1) \text{は真}\\
    &\forall n \in \bm{Z}^+ [P(n) \text{が真} \implies P(n+1) \text{も真}]
\end{align}
\setcounter{equation}{0}
このとき, 全ての $n \in \bm{Z}^+$ に対して, $P(n)$ が真となることを示せ.
\item $1$ は $\bm{Z}^+$ の最小元であることを示せ.
\item $S (\subset \bm{Z}^+)$ が次の条件を満たすとき, $S = \bm{Z}^+$ となることを示せ.
\begin{align}
    &1 \in S\\
    &\forall n \in \bm{Z}^+ [\forall k \in \bm{Z}^+[k < n \implies k \in S] \implies n + 1 \in S]
\end{align}
\setcounter{equation}{0}
\item (4) において, $S$ の条件 (2) における $k < n$ の部分を $k \leq n$ としても $S = \bm{Z}^+$ となることを示せ.
}{
\item $\bm{Z}^+ - S = S^{'}$ とし, $S^{'} = \emptyset$ となることを背理法により示す.\\
$S^{'} \neq \emptyset$ と仮定すると, $\bm{N}$ の整列性より $n_0 = \min S^{'}$ となる $n_0 \in \bm{N}$ が存在する. 今, $n_0 \in S^{'} \subset \bm{Z}^+$ より, $n_0 \neq 0$ であり, また,
$n_0 \notin S$ より, $n_0 \neq 1$ である. ここで, $n_0 < 1$ と仮定すると, $0$ が $\bm{N}$ の最小元であることと $n_0 \neq 0$ より, $0 < n_0 < 1 = \sigma (0)$ となるが, これは $\sigma (0)$ が $0$ の直後の元であることに矛盾する.
よって, $1 < n_0$ となり, 問\ref{sec:algebra}-\ref{pname:Nord}\ (4) より, $\sigma (1) \leq n_0$ となる.
ここで, $n_0 \neq 0$ より, $n_0 = \sigma ({n_0}^{'})$ となる ${n_0}^{'}$ が 存在する. この $n_0$ に対して, $\sigma (1) \leq \sigma ({n_0}^{'})$ となり, 問\ref{sec:algebra}-\ref{pname:Nord}\ (5) より $\sigma$ が順序単射であることから
...
群の定義.
\pbenumex{
空でない集合 $G$ と $G$ 上の二項演算（$*$ とする）が与えられ, $\langle G, *\rangle$ がモノイドであり, さらに次の条件 (G3) を満たすとき, $\langle G, *\rangle$ は群であるという.
\begin{align}
    & & \forall a \in G,\ \exists b \in G,\ a*b = b*a = e & & \text{($e$ は $G$ の $*$ に関する単位元)} \tag*{\hspace{3zw}(G3)}
\end{align}
また, $*$ が交換律を満たすとき, $\langle G, *\rangle$ は可換群という.
}{
\item 空でない集合 $X$ からそれ自身への全単射の集合を $G$ とする. 問 \ref{sec:set}-\ref{pname:fog}\ より, 任意の $G$ の元 $f, g$ に関して $f \circ g \in G$ となるので,
$f, g$ から $f \circ g$ を像とする写像を考えることができ, さらにこの写像は $G$ 上の二項演算である. 誤解が生じることがない場合には $\circ$ をこの演算と同一視する. 今, $\langle G, \circ \rangle$ は群となることを示せ.
\item 任意の集合 $S$ に対して, そのべき集合 $\mathcal{P}(S)$ を考える. 集合 $A, B$ に対して, $A \triangle B \coloneqq (A \backslash B) \cup (B \backslash A)$ と定義し, $A, B \in \mathcal{P}(S)$ に対して, $A \triangle B$ を対応させる写像も誤解のない場合は $\triangle$ で表すとき,
$\langle \mathcal{P}(S), \triangle \rangle$ は可換群となることを示せ.
}{
\item 問\ref{sec:set}-\ref{pname:fog}\ (3) より, $\circ$ が結合律を満たす. また, 任意の $f \in G$ をとると, $f \circ I_X = I_X \circ f = f$ となるので, (G2) を満たす.
最後に, $f \in G$ は全単射で, 問\ref{sec:set}-\ref{pname:fog}\ (2) とから, $f^{-1} \circ f = f \circ f^{-1} = I_X$ より, (G3) を満たす. よって, $\langle G, \circ \rangle$ 群となる.\\
この群は$X$ 上の対称群といい, 以後 $S(X)$ で表す.
\item まず, $\triangle$ が結合律を満たすことを示す. $x \in (A \triangle B) \triangle C$ とすると, $x \in (A \triangle B)\backslash C$ または $x \in C \backslash (A \triangle B)$ となる.
$x \in (A \triangle B)\backslash C$ のときは, $x \in (A \backslash B) \backslash C$ または $x \in (B \backslash A) \backslash C$ となる. $x \in C \backslash (A \triangle B)$ のときは,
$x \in C \land \lnot(x \in A\backslash B \lor x \in B \backslash A)$ で $x \in C \land ((x \notin A \lor x \in B) \land (x \notin B \lor x \in A))$ で $x \in C \land ((x \notin A \land x \notin B) \lor (x \in A \land x \in B))$ より,
$x \in (C\backslash B) \backslash A$ または $x \in A \cap B \cap C$ となる. あとは, これら 4 つの場合に対して, $x \in A \triangle (B \triangle C)$ を示せばよいが, めんどくさいのでいつかやる. 反対側も同様.
}
\makeatletter\tagsleft@false\makeatother
\end{nmprob}
...
指数表記.
\pbenum{
\item $\langle G, *\rangle$ を半群とする. $a_1a_2a_3 \cdots a_n \coloneqq (((a_1a_2)a_3)\cdots a_n)$ と定義する.\\
制作中
}{
\item 制作中
}
